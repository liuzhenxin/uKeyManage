 ...
 *
 */
static void sm4_one_round( const unsigned long sk[32],
...
int sms4_cbc_cipher(sm4_cipher_ctx *ctx, unsigned char *out, const unsigned char *in, size_t inl)
{
        while(inl>=((size_t)1<<(sizeof(long)*8-2)))
            {
	    sm4_crypt_cbc(&ctx->key, (ctx->key).mode, (long)((size_t)1<<(sizeof(long)*8-2)), ctx->iv, in, out);	    
            inl-=((size_t)1<<(sizeof(long)*8-2));
            in +=((size_t)1<<(sizeof(long)*8-2));
            out+=((size_t)1<<(sizeof(long)*8-2));
            }
        if (inl)
            sm4_crypt_cbc(&ctx->key, (ctx->key).mode, (long)inl, ctx->iv, in, out);		
        return 1;
}

int sms4_ecb_cipher(sm4_cipher_ctx *ctx, unsigned char *out, const unsigned char *in, size_t inl)
{
        sm4_crypt_ecb(&ctx->key, (ctx->key).mode, (long)inl, in, out);		
        return 1;
}
                      ...
                      const unsigned char *iv, int mode)
{
	...
	ret = sms4_init_key(ctx, key, SM4_ENCRYPT);
	...
	ctx->block_size = 16;
	ctx->mode = mode;
        ctx->block_mask = ctx->block_size - 1;
    return ret;
                     ...
                     const unsigned char *in, unsigned int inl)
{
	...
	if (ctx->buf_len == 0 && (inl & (ctx->block_mask)) == 0) {
        	if (ctx->mode == SM4_ECB) {
        		if (sms4_ecb_cipher(ctx, out, in, inl)){
        			*outl = inl;
        			return 1;
        		} 
        		else {
        	...
        	if (ctx->mode == SM4_CBC) {
        		if (sms4_cbc_cipher(ctx, out, in, inl)){
        			*outl = inl;
        			return 1;
        		} 
        		else {
    ...
    if (i != 0) {
	    ...
	    if ((ctx->mode == SM4_ECB) && (!sms4_ecb_cipher(ctx, out, ctx->buf, bl))) {
            	return 0;
            }
            if ((ctx->mode == SM4_CBC) && (!sms4_cbc_cipher(ctx, out, ctx->buf, bl))) {
                return 0;
            }
            inl -= j;
            in += j;
    ...
    inl -= i;
    if (inl > 0) {
        if ((ctx->mode == SM4_ECB) && (!sms4_ecb_cipher(ctx, out, in, inl))) {
            return 0;
        }
        if ((ctx->mode == SM4_CBC) && (!sms4_cbc_cipher(ctx, out, in, inl))) {
            return 0;
        }
...
int SM4_encrypt_final(sm4_cipher_ctx *ctx, unsigned char *out, unsigned int *outl)
{
    ...
    if ((ctx->mode == SM4_ECB) && (!sms4_ecb_cipher(ctx, out, ctx->buf, b))) {
        return 0;
    }
    else if ((ctx->mode == SM4_CBC) && (!sms4_cbc_cipher(ctx, out, ctx->buf,b))) {
        return 0;
    }
                      ...
                      const unsigned char *iv, int mode)
{
	...
	ret = sms4_init_key(ctx, key, SM4_DECRYPT);
                      ...
                      const unsigned char *in, unsigned int inl)
{
    ...
    if (!SM4_encrypt_update(ctx, out, outl, in, inl))
        return 0;
